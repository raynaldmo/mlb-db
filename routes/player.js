...
function PlayerHandler (cfg, db) {
    ...
    this.playerStats = function(req, res, next) {
        ...
        coll.find(query, {_id:0}).sort([["yearID", 1]]).toArray(function(err, result) {
            if (err) {
                return next(err);
            }
        ...
        if (position == 'P') {
            ...
            cursor = coll.aggregate([
                ...
                {$project : {
                   ...
                   IP : {$divide : ["$IPouts", 3]},
                   G : 1, W : 1, L : 1, H : 1, R : 1, ER : 1,
                   CG : 1, GS : 1, SV : 1, SHO : 1, K : 1, BB : 1
                }}
            ...
            cursor = coll.aggregate([
                ...
                {$project : {
                    Yrs : {$size : "$years"},
                    BA : {$divide : ["$H", "$AB"]},
                    OBP : {$divide : [ // on base percentage calculation
                            {$add : ["$H","$BB", "$HBP"]},
                            {$add : ["$AB", "$BB", "$HBP", "$SF"]}
                          ]},
                    ...
                    G : 1, AB : 1, R : 1, H : 1, BB : 1,
                    RBI : 1, HR : 1, SB : 1, "2B" : 1, "3B" : 1,
                    SO : 1
                }}
            ], {cursor : {batchSize : 1}} // appears that cursor and batchSize must be specified!
            );
    ...
    this.hofList = function(req, res, next) {
        ...
        cursor = hof.aggregate([
            ...
            {$project : {
                ...
                playerID : 1, inducted : 1, ballots : 1, votes : 1,
                category : 1
            }},
